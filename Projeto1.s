...
#2) Excluir despesa: excluir dados de uma despesa identificada pelo id informado pelo usuario
	la	$a0, msg_aidi	#mesagem de pegar id
	addi	$v0, $zero, 4
	syscall
	
	addi	$v0, $zero, 5	#codigo de ler int
	syscall
	
	add $s4, $zero, $v0	#$s4 possui o id
	...
	achei:
		...
		puxa:
			...
			sw	$t3, 6 ($s2)	#Ano
			s.s	$f1, 24 ($s2)	#Valor
			addi	$s2, $a3, -8	#Categoria
			
			addi 	$s2, $s2, -28
			
			bne	$s2, $s1, puxa
	
		la	$a0, msg_exc	#mensagem de excluido com sucesso
		addi	$v0, $zero, 4
		syscall
	...
	nao_achei:
		...
		la	$a0, msg_nao	#mensagem de despesa nao encontrada
		addi	$v0, $zero, 4
		syscall		
